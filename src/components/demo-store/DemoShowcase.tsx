...
const DemoShowcase = () => {
  ...
  return (
    <div className="relative flex flex-col lg:flex-row items-center justify-center gap-8 lg:gap-4 xl:gap-8">
      {demos.map((demo, i) => (
        <motion.div key={i} className="relative flex flex-col items-center justify-between px-3 sm:px-4 xl:px-5 3xl:px-8 pt-[18px] sm:pt-[30px] xl:pt-[18px] 3xl:pt-[26px] group bg-[#F1F4F7] rounded-2xl overflow-hidden"
        ...
        >
          ...
          <Link to={demo.link} className="block">
            ...
            {/* Overlay on hover */}
            <div className="absolute inset-0 rounded-2xl bg-black bg-opacity-0 opacity-0 group-hover:bg-opacity-40 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center">
              <ButtonDemoStore>View Demo</ButtonDemoStore>
            </div>
          </Link>
        </motion.div>
      ))}
